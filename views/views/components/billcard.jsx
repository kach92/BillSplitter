...
class BillCard extends React.Component {
  render() {
    ...
    if(user_id === payer_id){
        payment = <p className="amount-to-pay" style={{color:"green"}}>You lent <span>S${((parseFloat(this.props.amount)-parseFloat(this.props.split_amount))).toFixed(2)}{paidstatus}</span></p>
    }else{
        if(parseFloat(this.props.split_amount)===0){
            payment = <p className="amount-to-pay"style={{color:"grey"}}>You are not involved</p>
        }else{
            payment = <p className="amount-to-pay"style={{color:"red"}}>You borrowed <span>S${this.props.split_amount.toFixed(2)}{paidstatus}</span></p>
        }
